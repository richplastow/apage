...
(function() {
  ...
  tudor.equal([
    "Returned string is expected length", 2725, function(mock) {
      return mock.render().length;
    }, "Shorter `title` changes string length", 2713, function(mock) {
  ...
  tudor.equal([
    ...
    }, "The `path` field is rendered to inline script", 2670, function(mock) {
      return mock.append({
        path: 'abcxyz'
      }).render().indexOf('abcxyz');
    }, "Default `html` field is rendered to inline script", 2721, function(mock) {
      return mock.render().indexOf('@todo');
    }, "`html` field is parsed by markdown", 2706, function(mock) {
