...
/*! Apage 0.0.4 //// MIT Licence //// http://apage.richplastow.com/ */
...
(function() {
  ...
  Main = (function() {
    ...
    Main.prototype._rules = {
      config: [['title', 'Untitled', 'string', /^[^\x00-\x1F]{1,24}$/], ['content', '', 'string', /^[^\x00-\x08\x0E-\x1F]*$/]]
    };
    ...
    Main.prototype.appendContent = function(content) {
      var errors;
      content = this._c.content + content;
      if (errors = ª.populate({
        content: content
      }, this._c, this._rules.config, true)) {
  ...
  tudor.equal([
    "Returned object contains expected defaults", '{"title":"Untitled","content":""}', function(mock) {
  ...
  tudor.equal([
    "Returned string is expected length", 2675, function(mock) {
      return mock.render().length;
    }, "Shorter `title` changes string length", 2663, function(mock) {
